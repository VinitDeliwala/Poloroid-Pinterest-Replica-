...
{
	$count=$count+1;
	array_push($fgbname, $row['bname']);
}
$fgbnameCount = sizeOf($fgbname);
...
{
	$count=$count+1;
	array_push($fgbids, $row['bid']);
}
$fgbidCount = sizeOf($fgbids);

			for ($x = 0; $x < $fgbnameCount; $x++) {
				
				
				$query="select distinct mname,mid from repinboard natural join media where bid='$fgbids[$x]';";
				$result=$mysqli->query($query);
				$count=0;
				$mids = array();
				$mname = array();
				...
				//echo '<a href="id1.php?bid='.$fbids[$x]. '"><input type="button"  value='.$fbname[$x].'></a>';
							...
							{ 
										...
										$str3 = $str4.$str1.$str2;
										echo $str3;
										echo '<a href="id5.php?mid='.$mids[$y].'&bid='.$fgbids[$x].'"><img src="' . $str3. '">';
										echo "<p>$mname[$y]</p>";
